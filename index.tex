...
\section{パッケージを配ろう}
\label{sec:orga822f6b}
\begin{frame}[label={sec:org0cd3f78}]{お前誰よ}
\begin{block}{}
\begin{itemize}
...
\label{sec:orgd02eb6e}
\begin{frame}[label={sec:org066548e}]{パッケージエコシステム}
\begin{itemize}
\item 作る
\begin{itemize}
...
\begin{frame}[label={sec:org27c0b02}]{パッケージを配るということ}
\begin{itemize}
\item 広く一般に向けて配る
...
\begin{frame}[label={sec:orgffd0b97},fragile]{広く一般に向けてpypiで配る}
 \begin{itemize}
\item PyPAツールのデフォルト
...
\begin{frame}[label={sec:orga95feed}]{狭い範囲で限られた利用のために配る}
\begin{itemize}
...
\begin{frame}[label={sec:org90e28cf}]{狭い範囲で配る}
\begin{itemize}
...
\begin{frame}[label={sec:orgaeeec69},fragile]{httplib.server でのお手軽repository}
 \begin{itemize}
...
\begin{frame}[label={sec:orgee389c8},fragile]{URL指定でインストール}
 \begin{itemize}
...
\begin{frame}[label={sec:org0c1f420},fragile]{複雑なwheelファイル名}
 \begin{itemize}
\item oh\ldots{}
\end{itemize}
\begin{verbatim}
...
\end{verbatim}
\end{frame}
\begin{frame}[label={sec:org75cef17},fragile]{find-links}
 \begin{itemize}
...
\begin{frame}[label={sec:org9ebe70a},fragile]{no-index}
 \begin{itemize}
...
\begin{frame}[label={sec:org0ddf2cc},fragile]{indexは必要？}
 \begin{itemize}
...
\begin{frame}[label={sec:orga61eb83},fragile]{独自のpypiを立てたい!}
 \begin{itemize}
...
\label{sec:org923eb89}
\begin{frame}[label={sec:org8cc8124}]{パッケージを配るためのPEP}
\begin{itemize}
...
\begin{frame}[label={sec:orgb009032}]{Simple Repository}
representation
...
\begin{frame}[label={sec:org454fec4}]{PyPIのSimple Repository}
\begin{itemize}
...
\begin{frame}[label={sec:org1f35537}]{実装方針}
\begin{itemize}
\item 標準ライブラリでいこう
\begin{itemize}
...
\begin{frame}[label={sec:org16c942e},fragile]{project list}
 \begin{itemize}
...
\begin{frame}[label={sec:org0c71370},fragile]{使うライブラリ}
 \begin{itemize}
\item これだけ!
\item 100\% 標準ライブラリのみ!
\end{itemize}
...
\begin{frame}[label={sec:orgbe197a8},fragile]{Meta}
 \begin{itemize}
...
\begin{frame}[label={sec:orgba4742a}]{project detail}
\begin{itemize}
...
\begin{frame}[label={sec:orgb7db610},fragile]{project fileのtyping}
 \begin{itemize}
...
\begin{frame}[label={sec:org7f1880d},fragile]{project detailのtyping}
 \begin{itemize}
...
\begin{frame}[label={sec:org3a3daab},fragile]{project list のtyping}
 \begin{verbatim}
...
\begin{frame}[label={sec:org97ac171},fragile]{wheelファイルを探しだす}
 \begin{itemize}
\item pathlibでできちゃうね!
\end{itemize}
...
\begin{frame}[label={sec:orgf90c176},fragile]{wheelファイル名から情報を取得}
 \begin{itemize}
...
\begin{frame}[label={sec:org5db28e2},fragile]{wheelファイル名から情報を取得}
 \begin{itemize}
...
\begin{frame}[label={sec:org6e3a687},fragile]{プロジェクト名を正規化}
 \begin{itemize}
...
\begin{frame}[label={sec:org279f822},fragile]{metadata}
 \begin{itemize}
...
\begin{frame}[label={sec:orga95bf74},fragile]{全部まとめてwheelファイルの情報を取得}
 \begin{itemize}
...
\end{verbatim}
\end{frame}
\begin{frame}[label={sec:org8c52c06},fragile]{プロジェクトごとにファイル情報をまとめる}
 \begin{itemize}
...
\end{verbatim}
\end{frame}
\begin{frame}[label={sec:org7132cbb},fragile]{wsgiアプリケーション:project list}
 \begin{verbatim}
...
\end{verbatim}
\end{frame}
\begin{frame}[label={sec:org78b288f},fragile]{wsgiアプリケーション:project detail}
 \begin{verbatim}
...
\end{verbatim}
\end{frame}
\begin{frame}[label={sec:org089fa95},fragile]{wsgiアプリケーション:ダウンロード}
 \begin{itemize}
...
\begin{frame}[label={sec:org293c566},fragile]{WSGIアプリケーションのルーティング}
 \begin{itemize}
...
\end{verbatim}
\end{frame}
\begin{frame}[label={sec:orgc8a28c7},fragile]{さあ!wsgiアプリケーションを立ち上げよう!}
 \begin{itemize}
...
\section{まとめ}
\label{sec:orgd77eaa2}
\begin{frame}[label={sec:org4b3ae91}]{まとめ}
\begin{itemize}
\item パッケージの配布方法
\begin{itemize}
...
\begin{frame}[label={sec:org9f790a3}]{参考文献}
\begin{itemize}
