...
\section{パッケージを配ろう}
\label{sec:orgd7b0e1a}
\begin{frame}[label={sec:org4cea69b}]{お前誰よ}
\begin{block}{}
\begin{itemize}
...
\label{sec:orgc7adbaf}
\begin{frame}[label={sec:org7a4333d}]{パッケージエコシステム}
\begin{itemize}
\item 作る
\begin{itemize}
...
\begin{frame}[label={sec:org63db266}]{パッケージを配るということ}
\begin{itemize}
\item 広く一般に向けて配る
...
\begin{frame}[label={sec:orgb672961},fragile]{広く一般に向けてpypiで配る}
 \begin{itemize}
\item PyPAツールのデフォルト
...
\begin{frame}[label={sec:org6f2c137}]{狭い範囲で限られた利用のために配る}
\begin{itemize}
...
\begin{frame}[label={sec:orgf49ecb0}]{狭い範囲で配る}
\begin{itemize}
...
\begin{frame}[label={sec:org4fc00a9},fragile]{httplib.server でのお手軽repository}
 \begin{itemize}
...
\begin{frame}[label={sec:org511c543},fragile]{URL指定でインストール}
 \begin{itemize}
...
\begin{frame}[label={sec:org4f257b9},fragile]{複雑なwheelファイル名}
 \begin{itemize}
\item oh\ldots{}
\end{itemize}
\begin{verbatim}
...
\end{verbatim}
\end{frame}
\begin{frame}[label={sec:orgc855dd7},fragile]{find-links}
 \begin{itemize}
...
\begin{frame}[label={sec:org5755dd5},fragile]{no-index}
 \begin{itemize}
...
\begin{frame}[label={sec:org2750fd0},fragile]{indexは必要？}
 \begin{itemize}
...
\begin{frame}[label={sec:org538280c},fragile]{独自のpypiを立てたい!}
 \begin{itemize}
...
\label{sec:org61f317a}
\begin{frame}[label={sec:orge5db389}]{パッケージを配るためのPEP}
\begin{itemize}
...
\begin{frame}[label={sec:org28472d4}]{Simple Repository}
representation
...
\begin{frame}[label={sec:org4f14455}]{PyPIのSimple Repository}
\begin{itemize}
...
\begin{frame}[label={sec:org8477e70}]{実装方針}
\begin{itemize}
\item 標準ライブラリでいこう
\begin{itemize}
...
\begin{frame}[label={sec:orgb7bc350},fragile]{project list}
 \begin{itemize}
...
\begin{frame}[label={sec:org2d55ee5},fragile]{使うライブラリ}
 \begin{itemize}
\item これだけ!
\item 100\% 標準ライブラリのみ!
\end{itemize}
...
\begin{frame}[label={sec:org0641b27},fragile]{Meta}
 \begin{itemize}
...
\begin{frame}[label={sec:org71b7ec5}]{project detail}
\begin{itemize}
...
\begin{frame}[label={sec:orgec4336e},fragile]{project fileのtyping}
 \begin{itemize}
...
\begin{frame}[label={sec:org0a4488d},fragile]{project detailのtyping}
 \begin{itemize}
...
\begin{frame}[label={sec:orge100a48},fragile]{project list のtyping}
 \begin{verbatim}
...
\begin{frame}[label={sec:org99c54f9},fragile]{wheelファイルを探しだす}
 \begin{itemize}
\item pathlibでできちゃうね!
\end{itemize}
...
\begin{frame}[label={sec:org257082b},fragile]{wheelファイル名から情報を取得}
 \begin{itemize}
...
\begin{frame}[label={sec:org0119590},fragile]{wheelファイル名から情報を取得}
 \begin{itemize}
...
\begin{frame}[label={sec:org11be87f},fragile]{プロジェクト名を正規化}
 \begin{itemize}
...
\begin{frame}[label={sec:orgc8e62ab},fragile]{metadata}
 \begin{itemize}
...
\begin{frame}[label={sec:orgc48cb55},fragile]{全部まとめてwheelファイルの情報を取得}
 \begin{itemize}
...
\end{verbatim}
\end{frame}
\begin{frame}[label={sec:orgdfb1ce4},fragile]{プロジェクトごとにファイル情報をまとめる}
 \begin{itemize}
...
\end{verbatim}
\end{frame}
\begin{frame}[label={sec:orga25a4c0},fragile]{wsgiアプリケーション:project list}
 \begin{verbatim}
...
\end{verbatim}
\end{frame}
\begin{frame}[label={sec:org2ecc3e7},fragile]{wsgiアプリケーション:project detail}
 \begin{verbatim}
...
\end{verbatim}
\end{frame}
\begin{frame}[label={sec:orgff7f597},fragile]{wsgiアプリケーション:ダウンロード}
 \begin{itemize}
...
\begin{frame}[label={sec:org0db0302},fragile]{WSGIアプリケーションのルーティング}
 \begin{itemize}
...
\end{verbatim}
\end{frame}
\begin{frame}[label={sec:org185987a},fragile]{さあ!wsgiアプリケーションを立ち上げよう!}
 \begin{itemize}
...
\section{まとめ}
\label{sec:org6893ead}
\begin{frame}[label={sec:orge1b3d6c}]{まとめ}
\begin{itemize}
\item パッケージの配布方法
\begin{itemize}
...
\begin{frame}[label={sec:orged06df5}]{参考文献}
\begin{itemize}
